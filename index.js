...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Angga gans\n' // GANTI NAMA LU
            + 'ORG:OWNER BOTZ;\n'
            + 'TEL;type=CELL;type=VOICE;waid=6285266692786:+62 852-66692786\n' // GANTI NOMOR LU
            + 'END:VCARD'
// UDAH SEGITU KONSOL KEBAWAH BIARIN AJA
const ngonsol = JSON.parse(fs.readFileSync('./settings/Angga.json'))
const {
    botName,
    ownerName,
    XteamKey,
...
console.log(banner.string)
   ...
   qrcode.generate(qr, { small: true })
	console.log(color('[','white'), color('!','red'), color(']','white'), color(' MoshiBot id'))
})

	baby.on('credentials-updated', () => {
		fs.writeFileSync('./Angga.json', JSON.stringify(baby.base64EncodedAuthInfo(), null, '\t'))
		info('2', 'ingfokan cuyy...')
	})
	fs.existsSync('./Angga.json') && Moshi.loadAuthInfo('./Angga.json')
	baby.on('connecting', () => {
		start('2', 'Ramlan Connecting...')
	})
	baby.on('open', () => {
		success('2', 'Angga Connected')
	})
...
baby.on('group-participants-update', async (anu) => {
	...
	Moshi.on('message-new', async (mek) => {
		try {
			...
			const q = args.join(' ')
			const botNumber = Moshi.user.jid
			const sender = isGroup ? mek.participant : mek.key.remoteJid
			pushname = baby.contacts[sender] != undefined ? moshi.contacts[sender].vname || moshi.contacts[sender].notify : undefined
...
*/
			...
			const mentions = (teks, memberr, id) => {
				(id == null || id == undefined || id == false) ? moshi.sendMessage(from, teks.trim(), extendedText, {contextInfo: {"mentionedJid": memberr}}) : moshi.sendMessage(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": memberr}})
			}
          	...
          	let found = false
                    for (let lmt of _limit) {
                        if (lmt.id === sender) {
                            ...
                            if (limitCounts <= 0) return moshi.sendMessage(from,`Limit anda sudah habis\n\n_Note : limit bisa di dapatkan dengan cara ${prefix}buylimit dan naik level_`, text,{ quoted: mek})
                            moshi.sendMessage(from, nad.limitcount(limitCounts), text, { quoted : mek})
                            found = true
                        }
                    }
                    if (found === false) {
...
// ANTI LINK GRUP
		        ...
		        if (isGroupAdmins) return reply('Yang punya grub mah bebas:v')
		        ...
		        setTimeout( () => {
			        moshi.groupRemove(from, [kic]).catch((e)=>{reply(`BOT HARUS JADI ADMIN`)})
		        }, 3000)
		        setTimeout( () => {
			        moshi.updatePresence(from, Presence.composing)
			        reply("Hedsot :v")
		        }, 2000)
		        setTimeout( () => {
			        moshi.updatePresence(from, Presence.composing)
...
switch(command) {
				...
				case 'donasi':
				case 'donate':
					moshi.sendMessage(from, donasi(pushname, prefix, botName, ownerName), text)
					break
				case 'bingungcok':
				if (isBanned) return reply(nad.baned())
				if (!isRegistered) return reply(nad.noregis())				
...
// INGFO
                ...
                moshi.sendMessage(from, `Speed: ${latensi.toFixed(4)} _ms_`, text, { quoted: mek})
                    break
				case 'info':
					me = baby.user
					uptime = process.uptime()
					teks = `*➸ Nama bot* : ${me.name}\n*➸ OWNER* : ${ownerName}\n*➸ AUTHOR* : 𝗥𝗮𝗺𝗹𝗮𝗻 𝗜𝗗\n*➸ Nomor Owner* : @${ownerNumber('@')[0]}\n*➸ Prefix* : ${prefix}\n*➸ Total Block Contact* : ${blocked.length}\n*➸ The bot is active on* : ${kyun(uptime)}`
					buffer = await getBuffer(me.imgUrl)
					moshi.sendMessage(from, buffer, image, {caption: teks, contextInfo:{mentionedJid: [me.jid]}})
					break
            case 'admin':
            case 'owner':
            case 'creator':
                  moshi.sendMessage(from, {displayname: "Jeff", vcard: vcard}, MessageType.contact, { quoted: mek})
                  moshi.sendMessage(from, 'Tuh Nomor Pacarku >_<, Ehh Ownerku mksdnya:v',MessageType.text, { quoted: mek} )
					break
				case 'daftar':
				if (isBanned) return reply(nad.baned())
...
// SIMPLE MENU
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						const media = await moshi.downloadAndSaveMediaMessage(encmedia)
						ran = getRandom('.webp')
						await ffmpeg(`./${media}`)
							.input(media)
						...
						await ffmpeg(`./${media}`)
							...
							.on('end', function () {
								console.log('Finish')
								buffer = fs.readFileSync(ran)
								moshi.sendMessage(from, buffer, sticker, {quoted: mek})
								fs.unlinkSync(media)
								fs.unlinkSync(ran)
							})
							.addOutputOptions([`-vcodec`,`libwebp`,`-vf`,`scale='min(320,iw)':min'(320,ih)':force_original_aspect_ratio=decrease,fps=15, pad=320:320:-1:-1:color=white@0.0, split [a][b]; [a] palettegen=reserve_transparent=on:transparency_color=ffffff [p]; [b][p] paletteuse`])
						...
						reply(`Kirim gambar/video/gif dengan caption \n${prefix}sticker (durasi sticker video 1-9 detik)`)
							...
							break
// TTP
							case 'ttp': //by MoshiBot
				if (isBanned) return reply(nad.baned())
				if (!isRegistered) return reply(nad.noregis())
				if (isLimit(sender)) return reply(nad.limitend(pusname))
							if (args.length < 1) return baby.sendMessage(from, `Teksnya mana kak?\ncontoh : ${prefix}ttp botwea`, text, {quoted: mek})
							pngttp = './temp/ttp.png'
							...
							moshi.sendMessage(from, buffer, sticker)
...
// TTS
				...
				if (!isRegistered) return reply(nad.noregis())
				if (isLimit(sender)) return reply(nad.limitend(pusname))
				if (args.length < 1) return baby.sendMessage(from, `Kode bahasanya mana kak?\ncontoh : ${prefix}tts id Halo Mos`, text, {quoted: mek})
					const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return baby.sendMessage(from, `Teksnya mana kak?\ncontoh : ${prefix}tts id Halo Mos`, text, {quoted: mek})
					dtt = body.slice(8)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 300
...
// NULIS
				...
				nul = body.slice(7)
				reply('「❗」Bot Sedang Menulis Mohon Tunggu...')
				tak = await getBuffer(`https://api.zeks.xyz/api/nulis?text=${nul}&apikey=apivinz`)
				moshi.sendMessage(from, tak, image, {quoted: mek, caption: 'Lebih baik nulis sendiri ya kak :*'})
					await limitAdd(sender)				
				break
// QUOTES
				case 'quotes':
...
// BIKIN QUOTE
                ...
                var gh = body.slice(12)
					...
					var wm = gh.split("&")[1];
					const pref = `Teks nya mana kak?\n\ncontoh : ${prefix}bikinquote bot tamvan & botz`
					if (args.length < 1) return reply(pref)
					reply(nad.wait())
					anu = await fetchJson(`https://terhambar.com/aw/qts/?kata=${quote}&author=${wm}&tipe=random`, {method: 'get'})
					buffer = await getBuffer(anu.result)
					moshi.sendMessage(from, buffer, image, {caption: 'Nih kak >_<', quoted: mek})
					await limitAdd(sender)
					break
// STALKIG
                   case 'stalkig':
                   ...
                   if (!isRegistered) return reply(nad.noregis())
                   if (isLimit(sender)) return reply(nad.limitend(pusname))
                   if (args.length < 1) return reply(`Mau ngepoin siapa kak?\nContoh : ${prefix}stalkig iamramlan_`)
                     teks = body.slice(9)
                    ...
                    moshi.sendMessage(from, buffer, image, {quoted: mek, caption: hasil})
                    await limitAdd(sender)
			       break
//DOMPET
				case 'limit':
...
//TF
                ...
                addKoinUser('62852666927860@s.whatsapp.net', fee)
...
// DOWNLOAD
				...
				if (isLimit(sender)) return reply(nad.limitend(pusname))
					...
					reply('[WAIT] Proses Dulu Ya Gans)
					     infomp4 = `*「❗」Video Ditemukan「❗」*\n➸ Sumber : ${anu.result.name}\n➸ Caption : ${anu.result.caption}\n\n*[WAIT] Proses Dulu Ya Gans*`
					     oasu = await getBuffer(anu.result.cover)					
					mshi = await getBuffer(anu.result.result.server_1)
					moshi.sendMessage(from, oasu, image, {quoted: mek, caption: infomp4})
					moshi.sendMessage(from, rmln, video, {mimetype: 'video/mp4', quoted: mek})
					await limitAdd(sender)
					break
					case 'play':
                  if (isBanned) return reply(nad.baned())
				...
				if (isLimit(sender)) return reply(nad.limitend(pusname))
					     reply(nad.wait())
					     ngen = await fetchJson(`https://api.xteam.xyz/dl/play?lagu=${body.slice(6)}&APIKEY=${XteamKey}`)
					     if (ngen.error) return reply(ngen.error)
					     infomp3 = `*「❗」Lagu Ditemukan「❗」*\n➸ Judul : ${ngen.judul}\n➸ Size : ${ngen.size}\n\n*[WAIT] Proses Dumlu Yakan*`
					     bumfer = await getBuffer(ngen.thumbnail)
					     lamgu = await getBuffer(ngen.url)
					     moshi.sendMessage(from, bumfer, image, {quoted: mek, caption: infomp3})
					     moshi.sendMessage(from, lamgu, audio, {mimetype: 'audio/mp4', quoted: mek})
					     await limitAdd(sender)
					break
					case 'ytmp3':
					     if (isBanned) return reply(nad.baned())
					     ...
					     moshi.sendMessage(from, buffer, image, {quoted: mek, caption: infomp3})
					     moshi.sendMessage(from, lagu, audio, {mimetype: 'audio/mp4', quoted: mek})
					     await limitAdd(sender)
					break
					case 'igiphoto':
					     if (isBanned) return reply(nad.baned())
						 ...
						 if (isLimit(sender)) return reply(nad.limitend(pusname)) 
					     reply(nad.wait())
					     uh = await fetchJson(`https://api.xteam.xyz/dl/ig?url=${body.slice(10)}&APIKEY=${XteamKey}`)
					     if (uh.error) return reply(uh.error)
					     cuyy = `*Caption :* ${uh.result.caption}`
					     buffer = await getBuffer(uh.result.data[0].data)
					     moshi.sendMessage(from, buffer, image, {quoted: mek, caption: cuyy})
					     await limitAdd(sender)
					break
					case 'igvideo':
					     if (isBanned) return reply(nad.baned())
					     ...
					     moshi.sendMessage(from, ige, video, {mimetype: 'video/mp4', quoted: mek})
					     await limitAdd(sender)
					break
// GABUTZ MENU
				case 'tebakin':
				...
				if (isLimit(sender)) return reply(nad.limitend(pusname))
					...
					setTimeout( () => {
					moshi.sendMessage(from, tebak, text, {quoted: mek})
					}, 30000) // 1000 = 1s,
					setTimeout( () => {
				...
				if (isLimit(sender)) return reply(nad.limitend(pusname))
					...
					moshi.sendMessage(from, 'Pertanyaan : *'+bisakah+'*\n\nJawaban : '+ keh, text, { quoted: mek })
					await limitAdd(sender)
					break
				case 'kapankah':
				if (isBanned) return reply(nad.baned())
				...
				if (isLimit(sender)) return reply(nad.limitend(pusname))
					...
					moshi.sendMessage(from, 'Pertanyaan : *'+kapankah+'*\n\nJawaban : '+ koh, text, { quoted: mek })
					await limitAdd(sender)
					break
           case 'apakah':
           if (isBanned) return reply(nad.baned())
           ...
           if (isLimit(sender)) return reply(nad.limitend(pusname))
					...
					moshi.sendMessage(from, 'Pertanyaan : *'+apakah+'*\n\nJawaban : '+ kah, text, { quoted: mek })
					await limitAdd(sender)
					break
				case 'rate':
				if (isBanned) return reply(nad.baned())
				...
				if (isLimit(sender)) return reply(nad.limitend(pusname))
					...
					moshi.sendMessage(from, 'Pertanyaan : *'+rate+'*\n\nJawaban : '+ te+'%', text, { quoted: mek })
					await limitAdd(sender)
					break
           case 'hobby':
           if (isBanned) return reply(nad.baned())
           ...
           if (isLimit(sender)) return reply(nad.limitend(pusname))
					...
					moshi.sendMessage(from, 'Pertanyaan : *'+hobby+'*\n\nJawaban : '+ by, text, { quoted: mek })
					await limitAdd(sender)
					break					
           case 'seberapagay':
           if (isBanned) return reply(nad.baned())
		   ...
		   await limitAdd(sender)
				...
				if (isLimit(sender)) return reply(nad.limitend(pusname))
					...
					moshi.sendMessage(from, truteh, image, { caption: '*Truth*\n\n'+ ttrth, quoted: mek })
					await limitAdd(sender)
					break
                case 'dare':
                if (isBanned) return reply(nad.baned())
				...
				if (isLimit(sender)) return reply(nad.limitend(pusname))                
					...
					moshi.sendMessage(from, tod, image, { quoted: mek, caption: '*Dare*\n\n'+ der })
					await limitAdd(sender)
					break
                case 'cekbapak': // By Angga gans
                if (isBanned) return reply(nad.baned())
				if (!isRegistered) return reply(nad.noregis())
				if (isLimit(sender)) return reply(nad.limitend(pusname))
				const bapak =['Wah Mantap Lu Masih Punya Bapack\nPasti Bapack Nya Kuli :v\nAwowkwokwwok\n#CandabOs','Aowkwwo Disini Ada Yteam :v\nLu Yteam Bro? Awowkwowk\nSabar Bro Ga Punya Bapack\n#Camda','Bjir Bapack Mu Ternyata Sudah Cerai\nSedih Bro Gua Liatnya\nTapi Nih Tapi :v\nTetep Ae Lu Yteam Aowkwowkw Ngakak :v','Jangan #cekbapak Mulu Broo :v\nKasian Yang Yteam\nNtar Tersinggung Kan\nYahahaha Hayyuk By : angga gans']
					const cek = bapak[Math.floor(Math.random() * bapak.length)]
					moshi.sendMessage(from, cek, text, { quoted: mek})
					await limitAdd(sender)
					break
				case 'simi':
				if (isBanned) return reply(nad.baned())
...
// GROUP
				...
				if (isLimit(sender)) return reply(nad.limitend(pusname))			
                  ...
                  if (isBanned) return reply(nad.baned())					
					...
					if (args[0] === 'buka') {
						...
						moshi.groupSettingChange(from, GroupSettingChange.messageSend, false)
					} else if (args[0] === 'tutup') {
						reply(`*BERHASIL MENUTUP GROUP*`)
						moshi.groupSettingChange(from, GroupSettingChange.messageSend, true)
					}
					break
				case 'add':
                  if (isBanned) return reply(nad.baned())
				...
				if (!isRegistered) return reply(nad.noregis())	     	
					...
					if (mentioned.length > 1) {
						...
						moshi.groupRemove(from, mentioned)
					} else {
                ...
                if (isLimit(sender)) return reply(nad.limitend(pusname))
					...
					moshi.sendMessage(from, options, text)
					await limitAdd(sender)
					break					
                case 'level':
                  if (isBanned) return reply(nad.baned())                
               ...
               moshi.sendMessage(from, resul, text, { quoted: mek})
                .catch(async (err) => {
                        console.error(err)
                        await reply(`Error!\n${err}`)
                    })
				...
				if (isLimit(sender)) return reply(nad.limitend(pusname))                
				    ...
				    moshi.sendMessage(from, yeh, text, {quoted: mek})
			        await limitAdd(sender)
					break
				case 'tagall':
                  if (isBanned) return reply(nad.baned())
           ...
           case 'setname':
			    ...
			    if (!isGroupAdmins) return reply(nad.admin())
				if (!isBotGroupAdmins) return reply(nad.badmin())
                moshi.groupUpdateSubject(from, `${body.slice(9)}`)
                moshi.sendMessage(from, '「 SUKSES 」Mengubah Nama Grup', text, {quoted: mek})
					break
                case 'setdesc':
                if (!isRegistered) return reply(nad.noregis())                
                if (!isGroup) return reply(nad.groupo())
			    if (!isGroupAdmins) return reply(nad.admin())
				if (!isBotGroupAdmins) return reply(nad.badmin())
                moshi.groupUpdateDescription(from, `${body.slice(9)}`)
                moshi.sendMessage(from, '*「 SUKSES 」Mengubah Desk Grup', text, {quoted: mek})
					break
           case 'demote':
                if (!isRegistered) return reply(nad.noregis())           
					if (!isGroup) return reply(nad.groupo())
					...
					if (mentioned.length > 1) {
						...
						mentions(teks, mentioned, true)
						moshi.groupDemoteAdmin(from, mentioned)
					} else {
                ...
                if (!isRegistered) return reply(nad.noregis())				
					...
					if (mentioned.length > 1) {
						...
						mentions(teks, mentioned, true)
						moshi.groupMakeAdmin(from, mentioned)
					} else {
				...
				mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					...
					var bot = gh.split("&")[2];
					moshi.sendMessage(from, `${bot}`, text, {quoted: { key: { fromMe: false, participant: `${mentioned}`, ...(from ? { remoteJid: from } : {}) }, message: { conversation: `${target}` }}})
					await limitAdd(sender)
					break
                      case 'leave':
                      if (isBanned) return reply(nad.baned())      
                      ...
                      if (!isGroupAdmins) return reply(nad.admin())
                      setTimeout( () => {
                      moshi.groupLeave (from) 
                      }, 2000)
                      setTimeout( () => {
                      moshi.updatePresence(from, Presence.composing) 
                      if (!isRegistered) return reply(nad.noregis())
                      if (isBanned) return reply(nad.baned())   
                      moshi.sendMessage(from, 'Aku pamit dulu kak:)', text)
                      }, 0)
                      break
            	case 'mining':
                      if (!isRegistered) return reply(nad.noregis())
                      ...
                      reply(`karena Angga baik Bot memberikan ${one}Xp >_<`)
                      }else{
...
// MAKER MENU
		...
		case 'halloweentext':
           ...
           if (!isRegistered) return reply(nad.noregis())
           if (isLimit(sender)) return reply(nad.limitend(pusname))				
				if (args.length < 1) return reply(`Teksnya mana kak?\nContoh : ${prefix}halloweentext MoshiBot`)
				ween = body.slice(15)
		    ...
		    moshi.sendMessage(from, bumfer, image, {quoted: mek})
		    await limitAdd(sender)
		    break
		case 'smoketext':
           if (isBanned) return reply(nad.baned())
           if (!isRegistered) return reply(nad.noregis())
           if (isLimit(sender)) return reply(nad.limitend(pusname))
				if (args.length < 1) return reply(`Teksnya mana kak?\nContoh : ${prefix}smoketext Angga gans`)
				smoke = body.slice(11)
		    ...
		    moshi.sendMessage(from, bunfer, image, {quoted: mek})
		    await limitAdd(sender)
		    break
           case 'neontext':
           if (isBanned) return reply(nad.baned())
           ...
           moshi.sendMessage(from, butfer, image, {quoted: mek})
		    await limitAdd(sender)
           break
				case 'pornhub':
           if (isBanned) return reply(nad.baned())
           ...
           moshi.sendMessage(from, busser, image, {quoted: mek})
		    await limitAdd(sender)
           break
// OWNER
                case 'addprem':
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
						buff = await moshi.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							moshi.sendMessage(_.jid, buff, image, {caption: `*「 MOSHI BROADCAST 」*\n\n${body.slice(4)}`})
						}
						reply('')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `*「 MOSHI BROADCAST 」*\n\n${body.slice(4)}`)
						}
						reply('*「 SUKSES BOSKU 」*')
					}
					break
				case 'bcgc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						buff = await moshi.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							moshi.sendMessage(_.jid, buff, image, {caption: `*「 BC GROUP 」*\n\n➸ Dari Grup : ${groupName}\n➸ Pengirim : wa.me/${(sender.split('@')[0])}\n➸ Pesan : ${body.slice(6)}`})
						}
						reply('*「 SUKSES BOSKU 」*')
					} else {
						for (let _ of anu) {
				...
				case 'clearall':
				 ...
				 moshi.chatRead (from)
					if (!isGroup) return reply(nad.groupo())
					if (!isOwner) return reply(nad.ownerb())
					moshi.blockUser (`${body.slice(7)}@c.us`, "add")
					moshi.sendMessage(from, `perintah Diterima, memblokir ${body.slice(7)}@c.us`, text)
					break
                    case 'unblock':
					if (!isGroup) return reply(nad.groupo())
					if (!isOwner) return reply(nad.ownerb())
				    moshi.blockUser (`${body.slice(9)}@c.us`, "remove")
					moshi.sendMessage(from, `Perintah Diterima, membuka ${body.slice(9)}@c.us`, text)
					break   				
					case 'setppbot':
					if (!isOwner) return reply(nad.ownerb())
				    moshi.updatePresence(from, Presence.composing) 
				...
				case 'clone':
					...
					try {
						...
						moshi.updateProfilePicture(botNumber, buffer)
...
// RANDOM MENU
				case 'bokep':
				moehi.updatePresence(from, Presence.composing) 
                  if (isBanned) return reply(nad.baned())
				if (!isRegistered) return reply(nad.noregis())
				if (!isPrem) return reply(nad.premium())
                 ...
                 moshi.sendMessage(from, randBokep, image, {quoted: mek, caption: randTeks})
				break					
                case 'randomhentong':
                  if (isBanned) return reply(nad.baned())
				if (!isRegistered) return reply(nad.noregis())
				...
				reply('Tobat bro tobat :v')
					moshi.sendMessage(from, ahe, image, {quoted: mek, caption: 'Jangan Comly >_<'})
					await limitAdd(sender)
					break
					case 'erohentong':
				if (isBanned) return reply(nad.baned())
				...
				reply('Tobat bro tobat :v')
					moshi.sendMessage(from, ahu, image, {quoted: mek, caption: 'Jangan Comly >_<'})
					await limitAdd(sender)
					break
					case 'cuckold':
				if (isBanned) return reply(nad.baned())
				...
				reply('Tobat bro tobat :v')
					baby.sendMessage(from, agu, image, {quoted: mek, caption: 'Jangan Comly >_<'})
					await limitAdd(sender)
					break
					case 'zettahentong':
				if (isBanned) return reply(nad.baned())
				...
				reply('Tobat bro tobat :v')
					moshi.sendMessage(from, avu, image, {quoted: mek, caption: 'Jangan Comly >_<'})
					await limitAdd(sender)
					break
					case 'jahyhentong':
				if (isBanned) return reply(nad.baned())
				...
				reply('Tobat bro tobat :v')
					moshi.sendMessage(from, aku, image, {quoted: mek, caption: 'Jangan Comly >_<'})
					await limitAdd(sender)
					break
					case 'randommanga':
				if (isBanned) return reply(nad.baned())
				...
				reply('Tobat bro tobat :v')
					moshi.sendMessage(from, alu, image, {quoted: mek, caption: 'Jangan Comly >_<'})
					await limitAdd(sender)
					break
					case 'orgyhentong':
				if (isBanned) return reply(nad.baned())
				...
				reply('Tobat bro tobat :v')
					moshi.sendMessage(from, azu, image, {quoted: mek, caption: 'Jangan Comly >_<'})
					await limitAdd(sender)
					break
					case 'panties':
				if (isBanned) return reply(nad.baned())
				...
				reply('Tobat bro tobat :v')
					baby.sendMessage(from, axu, image, {quoted: mek, caption: 'Jangan Comly >_<'})
					await limitAdd(sender)
					break
					case 'wpwibu':
				if (isBanned) return reply(nad.baned())
                if (!isRegistered) return reply(nad.noregis())
                if (isLimit(sender)) return reply(nad.limitend(pusname))
				acu = await getBuffer(`https://api.xteam.xyz/randomimage/wpmobile?APIKEY=${XteamKey}`)
				reply(nad.wait())
					moshi.sendMessage(from, acu, image, {quoted: mek, caption: 'Jangan Comly >_<'})
					await limitAdd(sender)
					break
					case 'nsfwneko':
				if (isBanned) return reply(nad.baned())
				...
				reply('Tobat bro tobat :v')
					moshi.sendMessage(from, abu, image, {quoted: mek, caption: 'Jangan Comly >_<'})
					await limitAdd(sender)
					break
					case 'wpsfw':
				if (isBanned) return reply(nad.baned())
				...
				reply('Tobat bro tobat :v')
					moshi.sendMessage(from, anu, image, {quoted: mek, caption: 'Jangan Comly >_<'})
					await limitAdd(sender)
					break
					case 'sfwneko':
				if (isBanned) return reply(nad.baned())
				...
				reply('Tobat bro tobat :v')
					moshi.sendMessage(from, amu, image, {quoted: mek, caption: 'Jangan Comly >_<'})
					await limitAdd(sender)
					break
					case 'bdsmhentong':
				if (isBanned) return reply(nad.baned())
				...
				reply('Tobat bro tobat :v')
					moshi.sendMessage(from, aqu, image, {quoted: mek, caption: 'Jangan Comly >_<'})
					break
					case 'sangewibu':
				if (isBanned) return reply(nad.baned())
                if (!isRegistered) return reply(nad.noregis())
				...
				reply('Tobat bro tobat :v')
					baby.sendMessage(from, aau, image, {quoted: mek, caption: 'Jangan Comly >_<'})
					break
					case 'asshentong':
				if (isBanned) return reply(nad.baned())
                if (!isRegistered) return reply(nad.noregis())
				...
				reply('Tobat bro tobat :v')
					baby.sendMessage(from, asu, image, {quoted: mek, caption: 'Jangan Comly >_<'})
					break
					case 'thighs':
				if (isBanned) return reply(nad.baned())
                if (!isRegistered) return reply(nad.noregis())
				...
				reply('Tobat bro tobat :v')
					moshi.sendMessage(from, adu, image, {quoted: mek, caption: 'Jangan Comly >_<'})
					break
					case 'uniform':
				if (isBanned) return reply(nad.baned())
                if (!isRegistered) return reply(nad.noregis())
                if (!isPrem) return reply(nad.premium())
				afu = await getBuffer(`https://api.xteam.xyz/randomimage/uniform?APIKEY=${XteamKey}`)
				reply(nad.wait())
					moshi.sendMessage(from, afu, image, {quoted: mek, caption: 'Jangan Comly >_<'})
					break
// OTHER
                case 'moddroid':
                  if (isBanned) return reply(nad.baned())
			...
			moshi.sendMessage(from, buffer, image, {quoted: mek, caption: `${teks}`})
			break
			case 'happymod':
                  if (isBanned) return reply(nad.baned())
				if (!isRegistered) return reply(nad.noregis())
			...
			break
				...
				jsholat = body.slice(14)
                   jsh = await getBuffer(`https://api.xteam.xyz/jadwalsholat?kota=${jsholat}&APIKEY=${XteamKey}`)
                   tels = `*「 JADWAL SHOLAT 」*\n➸ Daerah : ${jsh.kota}\n\n◯ Subuh : ${jsh.Subuh}\n◯ Dzuhur : ${jsh.Dzuhur}\n◯ Ashar : ${jsh.Ashar}\n◯ Maghrib : ${jsh.Magrib}\n◯ Isya : ${jsh.Isha}\n\n➸ Tanggal : ${jsh.Tanggal}`
                   baby.sendMessage(from, tels, text, {quoted: mek})
                   await limitAdd(sender)
                   break
// SOUND					
case 'iri':
...
moshi.sendMessage(from, irimp3, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
await limitAdd(sender)
break
case 'pale':
                if (isBanned) return reply(nad.baned())
...
moshi.sendMessage(from, pa, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
await limitAdd(sender)
break
case 'sound1':
                if (isBanned) return reply(nad.baned())
...
moshi.sendMessage(from, satu, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
await limitAdd(sender)
break
case 'sound2':
                if (isBanned) return reply(nad.baned())
...
moshi.sendMessage(from, dua, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
await limitAdd(sender)
break
case 'sound3':
                if (isBanned) return reply(nad.baned())
...
moshi.sendMessage(from, tiga, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
await limitAdd(sender)
break
case 'sound4':
                if (isBanned) return reply(nad.baned())
...
moshi.sendMessage(from, lima, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
await limitAdd(sender)
break
case 'sound6':
                if (isBanned) return reply(nad.baned())
...
moshi.sendMessage(from, enam, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
await limitAdd(sender)
break
case 'sound7':
                if (isBanned) return reply(nad.baned())
...
moshi.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
await limitAdd(sender)
break
case 'sound8':
                if (isBanned) return reply(nad.baned())
...
moshi.sendMessage(from, lapan, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
await limitAdd(sender)
break
case 'sound9':
                if (isBanned) return reply(nad.baned())
...
moshi.sendMessage(from, bilan, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
await limitAdd(sender)
break
case 'sound10':
                if (isBanned) return reply(nad.baned())
...
moshi.sendMessage(from, puluh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
await limitAdd(sender)
break
// Script Ramlan ID
// Mengsedih ada yang mau numpang nama
